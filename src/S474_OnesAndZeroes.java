public class S474_OnesAndZeroes {
    ...
    public static void main(String[] args) {
        Solution solution = new S474_OnesAndZeroes().new Solution();
    ...
    class Solution {
        public int findMaxForm(String[] strs, int m, int n) {
            ...
            for (String s : strs) {
                int[] calcNum = calcNum(s);
                for (int i = m; i >= calcNum[0]; i--) {
                    for (int j = n; j >= calcNum[1]; j--) {
                        dp[i][j] = Math.max(dp[i][j], dp[i - calcNum[0]][j - calcNum[1]] + 1);
                    }
                }
            }
            return dp[m][n];
